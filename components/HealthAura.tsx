...
const HealthAura: React.FC<HealthAuraProps> = ({ status, score }) => {
  ...
  return (
    <div className="relative flex items-center justify-center w-64 h-64 mx-auto group cursor-pointer">
      {/* Outer Pulse Rings */}
      ...
      {/* Main Aura Ring */}
      <div className={`relative w-48 h-48 rounded-full bg-gradient-to-br ${colors[status]} p-[2px] ${glowColors[status]} shadow-2xl transition-all duration-700 group-hover:scale-105`}>
        <div className="w-full h-full rounded-full bg-[#0A0A0B] flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <span className="text-xs font-bold text-neutral-500 tracking-widest uppercase mb-1">Health Index</span>
          <span className="text-6xl font-bold text-white tracking-tighter">{score}</span>
          <div className={`mt-2 px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase border border-white/10 bg-white/5`}>
            {status}
          </div>
        </div>
      </div>
      ...
      {/* Status Indicators */}
      <div className="absolute -top-4 -right-4 glass px-3 py-1 rounded-full text-[10px] font-mono text-indigo-400">
        Active Sentinel
      </div>
    </div>
  );
